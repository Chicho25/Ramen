...
// Function to draw HTML table within lookup POPUP
	...
	*/
	...
	function getPOQty($pono)
	{
		$temp = MySQLQuery("SELECT sum(purchase_detail.qty) as totQty from purchase_order 
                            inner join purchase_detail on purchase_detail.id_po = purchase_order.id
                            where po_no = ".$pono);
		$rstRow = mysql_fetch_array($temp);
		$totQty =  $rstRow['totQty'];
	...
	function getROItemQty($pono, $itmid)
	{
		$temp = MySQLQuery("SELECT sum(receive_detail.RecvQty) as RecvQty from receive_order 
                            inner join receive_detail on receive_detail.id_ro = receive_order.id
                            where po_no = ".$pono." and receive_detail.id_item = ". $itmid);
		$rstRow = mysql_fetch_array($temp);
		if(!$rstRow['RecvQty'])
			$RecvQty =  0;
		else
			$RecvQty =  $rstRow['RecvQty'];
	...
	function getInventoryItemQty($warehouseid, $itmid)
	{
		$temp = MySQLQuery("SELECT sum(qty) as totQty from inventory_adjustment
                            where id_warehouse = ".$warehouseid." and id_item = ". $itmid);
		$rstRow = mysql_fetch_array($temp);
		if(!$rstRow['totQty'])
			$RecvQty =  0;
		else
			$RecvQty =  $rstRow['totQty'];
	
		return $RecvQty;

	}

	function cantidadEnInventario($itmid){
		$temp = MySQLQuery("SELECT sum(qty) as totQty from inventory_adjustment
												where id_item = ". $itmid);
		$rstRow = mysql_fetch_array($temp);
		if(!$rstRow['totQty'])
...
function drawComboBox($nSelectedVal, $strName, $arr)
{
	...
	*/
	...
	function maxAdjNumber()
	{
		$strQuery = "Select MAX(order_no)	as order_no from  inventory_adjustment";
		
		$nRes = MySQLQuery($strQuery);				
		
		$nRows = mysql_fetch_array($nRes);
		$order_no = $nRows['order_no'];
		if ($order_no=="")
		{
			 $order_no = 1;
		}
	...
	function maxPONumber()
	{
		$strQuery = "Select MAX(po_no)	as order_no from  purchase_order";
	...
	function maxWONumber()
	{
		$strQuery = "Select MAX(wo_no)	as maxV from workorder";
