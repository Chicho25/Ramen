<?php 
    ...
    $where = "where (1=1)";
      ...
      {
        $where.=" and  (vehicle.name LIKE '%".$_POST['cname']."%' OR documents.createdOn LIKE '%".$_POST['cname']."%' OR documents.name LIKE '%".$_POST['cname']."%' OR documents.description LIKE '%".$_POST['cname']."%' )";
        $name = $_POST['cname'];
      }
      if(isset($_POST['status']) && $_POST['status'] != "")
      {
        $where.=" and  documents.stat =  ".$_POST['status'];
        $status = $_POST['status'];
      }
      else
      {
        $where.=" and  documents.stat =  1";
        $status = 1;
      }
      $arrUser = GetRecords("SELECT documents.*, vehicle.name as VehName
                            from documents
                            inner join vehicle on vehicle.id = documents.id_vehicle
                        ...
                        <div class="row wrapper ">
                          <div class="col-sm-3 pull-left">
                            <span class="input-group-btn padder ">
                              <button type="button" class="btn btn-success btn-rounded" onclick="window.location='register-document.php'"?>Add Document Entry</button>
                            </span>
                          </div>
                            ...
                            <table class="table table-striped table-bordered table-hover dataTables-example">
                              ...
                              <tr> 
                                  ...
                                  <td> 
                                      ...
                                      <button type="button" onclick="window.location='edit-document.php?id=<?php echo $value['id']?>';" class="btn green btn-info">Edit</button> 
                                      <button type="button" onclick="window.location='view-document.php?id=<?php echo $value['id']?>';" class="btn green btn-info">View</button> 
                                  </td>
                              </tr>
                              <?php
                                $i++;
