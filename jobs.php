<?php 
    ...
    $where = "where (1=1)";
      ...
      {
        $where.=" and  (ticketid LIKE '%".$_POST['cname']."%' OR project LIKE '%".$_POST['cname']."%' OR customer.name LIKE '%".$_POST['cname']."%' OR contact.name LIKE '%".$_POST['cname']."%' )";
        $name = $_POST['cname'];
      }
      if(isset($_POST['status']) && $_POST['status'] != "")
      {
        $where.=" and  jobs.stat =  ".$_POST['status'];
        $status = $_POST['status'];
      }
      else
      {
        $where.=" and  jobs.stat =  1";
        $status = 1;
      }
      $arrUser = GetRecords("SELECT jobs.*, contact.name as contactName, customer.name as CustName
                            from jobs
                            inner join contact on contact.id = jobs.id_contact
                        ...
                        <div class="row wrapper ">
                          <div class="col-sm-3 pull-left">
                            <span class="input-group-btn padder ">
                              <button type="button" class="btn btn-success btn-rounded" onclick="window.location='register-job.php'"?>Add Job</button>
                            </span>
                          </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover dataTables-example">
                              ...
                              <?PHP  
                                ...
                                ?> 
                              <tr> 
                                  <td class="tbdata"> <?php echo $value['ticketid']?> </td>
                                  <td class="tbdata"> <?php echo $value['project']?> </td>
                                  <td class="tbdata"> <?php echo $value['CustName']?> </td>
                                  <td class="tbdata"> <?php echo $value['contactName']?> </td>
                                  <td class="tbdata"> <?php echo $status?> </td>
                                  <td> <button type="button" onclick="window.location='edit-job.php?id=<?php echo $value['id']?>';" class="btn green btn-info">Edit</button> 
                                  </td>
                              </tr>
                              <?php
                                $i++;
